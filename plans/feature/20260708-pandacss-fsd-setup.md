...
- [ ] `pnpm install` 시 styled-system 자동 생성 (클린 클론 기준)
- [ ] `pnpm dev` → 홈 화면 + API 연결 상태 회귀 없음
- [ ] 다크/라이트 모드 스타일 회귀 없음 (Panda 전환 후)
- [ ] `pnpm build` 통과 (web, api)
- [ ] `pnpm lint` + FSD 경계 검사 통과
- [ ] CSS Modules 파일 잔존 0개 (결정 3에서 전부 전환 선택 시)
