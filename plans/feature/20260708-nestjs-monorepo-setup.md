...
- [x] `pnpm install` 클린 설치 성공
- [x] `pnpm dev:web` → 기존 Next 화면 정상 (구조 이동 후 회귀 없음, HTTP 200)
- [x] `pnpm dev:api` → `GET /api/health` 200 (`{"status":"ok","service":"api",...}`)
- [x] `pnpm dev` → 프론트 페이지에 "API 연결됨" 표시, api 종료 시 "API 연결 안 됨" 표시
- [x] `pnpm lint` 통과 (web, api 모두)
- [x] 커밋이 단계별로 분리되어 있고 각 커밋 시점에 빌드 가능 (`pnpm build` 통과)
